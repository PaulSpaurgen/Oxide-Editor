...
export default function Timeline() {
  ...
  const formatTickLabel = useCallback((totalSeconds: number, majorTickSeconds: number): string => {
  ...
  const drawTicks = useCallback(() => {
    ...
    const width = Math.min(Math.max(containerWidth, timelineSecondsBasedwidth), maxReasonableWidth);
    ...
    <div className="w-full bg-neutral-900 text-white select-none">
      ...
      <div className="p-2 flex items-center gap-4">
        ...
        <input
          ...
          value={zoom}
          onChange={(e) =>
            setZoom(parseInt(e.target.value) as ZoomLevel)
          }
          className="w-48"
        />
        <span>{zoom}x</span>
      ...
      >
        <div className="pl-4 flex" style={{ minWidth: "100%" }}>{ticks}</div>
